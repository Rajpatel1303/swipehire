...
export const WhatsAppOperationsPage: React.FC = () => {
  ...
  const loadData = useCallback(async (showRefreshing = false) => {
    ...
    try {
      ...
      console.error("[WhatsAppOperationsPage Load Error]:", err);
      setErrorMsg(err.message || "Failed to load WhatsApp dispatch logs.");
    } finally {
      setIsLoading(false);
  ...
  const columns: Column<CommunicationLogRecord>[] = [
    {
      ...
      render: (log) => {
        // Mask phone number for privacy: e.g. +91 98765 ****0
        const raw = log.recipient;
        const masked = raw.length > 5 ? raw.slice(0, 5) + "••••" + raw.slice(-2) : raw;
        return (
          <div>
            <span className="font-mono font-bold text-slate-900 block text-xs">{masked}</span>
            <span className="text-[10px] text-slate-500 font-mono">From: {log.sender || "Recruiter"}</span>
          </div>
        );
      },
    },
    {
      ...
      render: (log) => (
        <span className="text-xs font-medium text-slate-800">
          {log.template_name || "Direct Outreach"}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (log) => {
        ...
        return <span className="text-xs text-emerald-600 font-medium">Delivered to Candidate</span>;
      },
    },
    {
      key: "created_at",
  ...
  const waStats = metrics?.whatsapp || { total: 0, sent: 0, failed: 0 };
  const failureRate = waStats.total > 0 ? Math.round((waStats.failed / waStats.total) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium mt-0.5">
            Candidate WhatsApp outreach logs, direct recruiter conversations, and deliverability.
          </p>
        </div>
      ...
      {/* Safety Note */}
      <div className="p-3.5 bg-teal-50/50 border border-teal-200 rounded-2xl flex items-center gap-3 text-xs text-teal-900">
        <ShieldCheck className="w-5 h-5 text-teal-600 shrink-0" />
        <div>
          <span className="font-bold">Anti-Spam & Operational Security Notice:</span> Provider credentials and tokens are strictly concealed. Mass unsolicited messaging is structurally prevented to safeguard WhatsApp sender trust scores.
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={waStats.total.toLocaleString()}
          icon={MessageSquare}
          subtitle="Outbound candidate messages"
          colorVariant="primary"
        />
        <StatCard
          title="Delivered Successfully"
          value={waStats.sent.toLocaleString()}
          icon={CheckCircle2}
        ...
        <StatCard
          ...
          value={waStats.failed.toLocaleString()}
          icon={XCircle}
          subtitle={`${failureRate}% failure rate`}
          colorVariant={waStats.failed > 0 ? "danger" : "neutral"}
        />
        <StatCard
          title="Provider Integration"
          value="In-App Web"
          icon={Clock}
          subtitle="Direct WhatsApp Protocol"
      ...
      {errorMsg && (
        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs flex items-center gap-3">
          ...
          <div>
            <p className="font-bold">Error loading WhatsApp logs</p>
            <p>{errorMsg}</p>
          </div>
        </div>
      )}
      ...
      <DataTable
        ...
        keyExtractor={(l) => l.id}
        isLoading={isLoading}
        emptyMessage="No WhatsApp dispatch records found matching your filters."
        pagination={{
          currentPage: page,
          pageSize,
          totalItems: total,
          ...
          <div className="space-y-4 text-xs">
            <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200">
              <div>
                ...
                <span className="font-mono font-bold text-slate-900">{inspectingLog.recipient}</span>
              </div>
              <div>
            ...
            <div>
              <span className="text-[10px] text-slate-500 font-bold uppercase block mb-1">Template / Message</span>
              <div className="p-3 bg-white border border-slate-200 rounded-xl font-medium text-slate-800">
                {inspectingLog.subject || inspectingLog.template_name || "(Direct Message Outreach)"}
              </div>
            </div>

            {inspectingLog.error_message && (
              <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-rose-900">
                <span className="text-[10px] font-bold uppercase block text-rose-700 mb-1">Diagnostic Failure Reason</span>
                <p className="font-mono">{inspectingLog.error_message}</p>
              </div>
            )}

            <div>
              <span className="text-[10px] text-slate-500 font-bold uppercase block mb-1">Safe Telemetry Metadata</span>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to retry message dispatch to ${retryingLog?.recipient}? The original validated operation will be marked as retried.`}
